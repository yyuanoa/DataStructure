...
int AvlTree<T>::GetHeight(AvlNode<T> *t)
{
    /*
        ...
        叶子结点的高度为0，插入第一个结点时，height = max(-1, -1) + 1 = 0
    */
    if (t == NULL)
        return -1;
    else
...
void AvlTree<T>::Insert(AvlNode<T>* &t, T x)
{
    ...
    else if (x < t->data)
    {
        Insert(t->left, x);
        //判断平衡情况
        if (GetHeight(t->left) - GetHeight(t->right) > 1)  // 说明最后一个结点插入在左子树上
        {
            //分两种情况 左左或左右
    ...
    t->height = max(GetHeight(t->left), GetHeight(t->right)) + 1;  // 获取t的高度
}
...
int main()
{
    vector<int> nums = {10, 5, 15,13, 17, 11};
    // vector<int> nums = {10, 6, 3, 8, 15};
    AvlTree<int> tree;
